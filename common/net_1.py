...
class PConv(chainer.Chain):
    def __init__(self, ch0, ch1, bn=True, sample='none-3', activation=F.relu, dropout=False, noise=False):
        ...
        if sample=='down-5':
            layers['c'] = L.Convolution2D(2*ch0, ch1, 5, 2, 2, initialW=w)
            layers['c1'] = L.Convolution2D(ch0, ch0, 5, 1, 2, initialW=w)
            #layers['c1'] = VGG(ch0)
            layers['c2'] = DenseBlock(ch0, ch0, 1,None)
            layers['m'] = L.Convolution2D(2*ch0, ch1, 5, 2, 2, initialW=1.0, nobias=True)
            layers['m1'] = L.Convolution2D(ch0, ch0, 5, 1, 2, initialW=w)
            layers['m2'] = DenseBlock(ch0, ch0, 1,None)
        elif sample=='down-7':
            layers['c'] = L.Convolution2D(2*ch0, ch1, 7, 2, 3, initialW=w)
            layers['c2'] = DenseBlock(ch0, ch0, 1,None)
            ...
            layers['m1'] = L.Convolution2D(ch0, ch0, 7, 1, 3, initialW=w)
            layers['m2'] = DenseBlock(ch0, ch0, 1,None)
        elif sample=='down-3':
            layers['c'] = L.Convolution2D(2*ch0, ch1, 3, 2, 1, initialW=w)
            layers['c1'] = L.Convolution2D(ch0, ch0, 3, 1, 1, initialW=w)
            #layers['c1'] = VGG(ch0)
            layers['c2'] = DenseBlock(ch0, ch0, 1,None)
            layers['m'] = L.Convolution2D(2*ch0, ch1, 3, 2, 1, initialW=1.0, nobias=True)
            layers['m1'] = L.Convolution2D(ch0, ch0, 3, 1, 1, initialW=w)
            layers['m2'] = DenseBlock(ch0, ch0, 1,None)
        else:
            layers['c'] = L.Convolution2D(2*ch0, ch1, 3, 1, 1, initialW=w)
            layers['c1'] = L.Convolution2D(ch0, ch0, 3, 1, 1, initialW=w)
            layers['c2'] = DenseBlock(ch0, ch0, 1,None)
    ...
    def __call__(self, x, mask):
        ...
        if self.sample == 'down-2'or self.sample == 'down-1':
            ...
            mask_sums_2 = self.m1(mask_1)
            mask_sums_1 = self.m2(mask_sums_2)
