...
def main():
    ...
    parser.add_argument('--eval_folder', '-e', default='CtoP_generated_results_'+number,
                        help='Directory to output the evaluation result')
    #parser.add_argument("--load_model",  default='./result/model1200000+600000.npz', help='completion model path')
    parser.add_argument("--load_model",  default='./C_model1350000.npz', help='completion model path')
    ...
    val_dataset = getattr(datasets, args.load_dataset)(paths.test_place2,mask_path="./datasets/place2_256/mask_dataset/"+number)

    for iii in range(1249):     #(valid總照片/8)
    ...
    # generate results
        ...
        # Image.fromarray(img3).save(args.eval_folder + "/generated_Iout_{}.jpg".format(i))
            #scipy.misc.imsave(args.eval_folder + "/generated_Iout{}.jpg".format(i+3*iii), img3)
            z = str(i+3*iii + 1).zfill(8)
            scipy.misc.imsave(args.eval_folder + "/generated_Iout_{}.jpg".format(z), img3)
